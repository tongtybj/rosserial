#ifndef _ROS_hydrus_transform_control_PMatrixPseudoInverseWithInertia_h
#define _ROS_hydrus_transform_control_PMatrixPseudoInverseWithInertia_h

#include <stdint.h>
#include <string.h>
#include <stdlib.h>
#include "ros/msg.h"
#include "hydrus_transform_control/PMatrixPseudoInverseUnit.h"
...
namespace hydrus_transform_control
{
  ...
  class PMatrixPseudoInverseWithInertia : public ros::Msg
  {
    public:
      ...
      hydrus_transform_control::PMatrixPseudoInverseUnit st_pseudo_inverse;
      hydrus_transform_control::PMatrixPseudoInverseUnit * pseudo_inverse;
      int16_t inertia[6];
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(pseudo_inverse_lengthT > pseudo_inverse_length)
        this->pseudo_inverse = (hydrus_transform_control::PMatrixPseudoInverseUnit*)realloc(this->pseudo_inverse, pseudo_inverse_lengthT * sizeof(hydrus_transform_control::PMatrixPseudoInverseUnit));
      offset += 3;
      pseudo_inverse_length = pseudo_inverse_lengthT;
      for( uint8_t i = 0; i < pseudo_inverse_length; i++){
      offset += this->st_pseudo_inverse.deserialize(inbuffer + offset);
        memcpy( &(this->pseudo_inverse[i]), &(this->st_pseudo_inverse), sizeof(hydrus_transform_control::PMatrixPseudoInverseUnit));
      }
      for( uint8_t i = 0; i < 6; i++){
      union {
        int16_t real;
    ...
    const char * getType(){ return "hydrus_transform_control/PMatrixPseudoInverseWithInertia"; };
